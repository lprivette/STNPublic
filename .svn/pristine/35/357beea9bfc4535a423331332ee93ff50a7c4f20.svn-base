...
*/
dojo.provide("modules.SiteFilter");
...
dojo.declare("modules.SiteFilter", [ dijit._WidgetBase, dijit._OnDijitClickMixin, dijit._Container, dijit._TemplatedMixin],
{
	templatePath: dojo.moduleUrl("modules", "templates/SiteFilter.html"),
	...
	_onIconClick: function() {
		if (dojo.getStyle("filterBoxContent", "display") == "none") {
			dojo.fx.wipeIn({ node: "filterBoxContent", duration: 300}).play(); //Animate show content
			
		} else {
			  dojo.fx.wipeOut({node: "filterBoxContent", duration: 300}).play(); // Animate hide content
